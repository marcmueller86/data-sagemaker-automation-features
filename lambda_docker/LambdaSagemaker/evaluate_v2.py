...
def plot_feature_importance(model):
    feature_names = []
    try:
        with open('/opt/ml/processing/features/feature_names.json', 'r') as file:
            feature_names = json.load(file)
    except Exception as e:
        logger.info("No feature names found: {}".format(e))

    # Retrieve feature importances from the model
    feature_importance_dict = model.get_score(importance_type='weight')

    # Map feature importances to the actual feature names if possible
    if feature_names:
        feature_importance = {feature_names[int(f[1:])] : importance for f, importance in feature_importance_dict.items()}
    else:
    ...
    df = pd.DataFrame(sorted_feature_importance, columns=["feature_name", "importance_value"])
    df.to_csv("/opt/ml/processing/evaluation/feature_importance.csv", index=False)

    # Plotting feature importance
    plt.figure(figsize=(10, len(df) * 0.5))  # Adjust the height based on the number of features

    # Temporarily update model's feature names for plotting
    original_feature_names = model.feature_names
    model.feature_names = [fn for fn, _ in sorted_feature_importance]

    ax = xgboost.plot_importance(model, importance_type='weight', height=0.5, title='Top 50 Feature Importances')
    # If you want to show the real names in the plot, ensure feature_names are set correctly in the model
    # Reset feature names to original if necessary
    model.feature_names = original_feature_names

    plt.tight_layout()
    plt.savefig("/opt/ml/processing/evaluation/top_feature_importance.png", bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    feature_name = "/opt/ml/processing/features/feature_names.json"
    ...
    try:
        ...
        logger.info(f"Error features: {str(e)}")
        logger.info(f"Error features: {feature_name}")
    ...
    try:
        ...
        logger.info(f"Error test_set_mapping: {str(e)}")
        logger.info(f"Error test_set_mapping: {test_set_mapping}")
