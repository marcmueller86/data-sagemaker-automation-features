...
def log_varibale_type(variable):
    print (f"variable: {variable}, {type(variable)}" )
...
def get_pipeline_inference(
    ...
    default_bucket='s3://sagemaker-eu-central-1-812419731335',
    role=None,
...
#     latest_model_name = sm.describe_model_package(ModelPackageName=model_package_arn)['InferenceSpecification']['Containers'][0]['ModelDataUrl']
    ...
    step_transform = TransformStep(
    name="PLXClassificationTransform", 
    transformer=transformer, inputs=TransformInput(Join(on='/', values=['s3://px-data-ml-ops-data-prod', 'features', base_job_prefix, use_case, 'features_inference.csv']), split_type='Line',content_type='text/csv', input_filter='$[1:]', join_source='Input', output_filter='$[0,-1]')
)
